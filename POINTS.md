...
- 構造体の種類
  ...
  - タプル型構造体
  - ユニット型構造体
- 名前付きフィールド型構造体とタプル型構造体はメモリ上では同じ
- `#[reptr(C)]`属性をつけることでCとC++と互換性のある方法で、構造体をメモリ上に配置することができる。
- 関連関数：implブロック内で特定の方に関連づけられている関数
- 自由関数：implブロック外で定義された関数
- 型関連関数：implで定義されたselfを引数にとならない関数
- １つの方に対して複数のimplブロックを書くこともできるが、すべてのimplブロックが、その方を定義ｓ他のと同じクレートに入っていなければならない
- 型とメソッドを別におく場所のメリット
  - 型のデータメンバを見つけるのが簡単
  - タプル型やユニット型の構造体にメソッドを綺麗に定義できる
  - i32やenumにもimplブロックが使える
  - impl構文でトレイトの実装もできる